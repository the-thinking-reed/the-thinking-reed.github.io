---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

<!-- {% if site.google_scholar_stats_use_cdn %}
{% assign gsDataBaseUrl = "https://cdn.jsdelivr.net/gh/" | append: site.repository | append: "@" %}
{% else %}
{% assign gsDataBaseUrl = "https://raw.githubusercontent.com/" | append: site.repository | append: "/" %}
{% endif %}
{% assign url = gsDataBaseUrl | append: "google-scholar-stats/gs_data_shieldsio.json" %} -->

<span class='anchor' id='about-us'></span>

---

我们来自东南大学信息科学与工程学院，由**徐寅飞**老师带领创建，是一群痴迷于"信息"本身的人。

**徐寅飞老师简介**

徐寅飞，东南大学信息科学与工程学院副研究员、博士生导师。2008年、2016年分别于东南大学获学士和博士学位，2014–2015年赴加拿大麦克马斯特大学访问，2016–2017年于香港中文大学网络编码研究所从事博士后研究。在*IEEE Transactions on Information Theory*、ISIT等顶级期刊会议发表论文40余篇，研究方向聚焦无线通信与统计学习的基础理论问题。[[Google Scholar]](https://scholar.google.com/citations?user=jagXzu8AAAAJ)
<!-- <a href=''><img src="https://img.shields.io/endpoint?url={{ url | url_encode }}&logo=Google%20Scholar&labelColor=f6f6f6&color=9cf&style=flat&label=引用"></a> -->

**我们在做什么？**

- **网络信息论** — 探索多终端通信网络的极限，研究有损压缩、安全通信等经典问题的理论边界
- **信息年龄（Age of Information）** — 让信息"保鲜"，优化实时系统中的更新策略与调度机制
- **时序预测与数据压缩** — 用更少的比特描述世界，让预测更准、传输更快
- **AI Agent** — 打造能自主决策的智能体，也尝试用生成式AI做点有趣的事


<span class='anchor' id='-xl'></span>

# 🔥 News
- *2026.02*, 🎉🎉 Welcome **Xiaoyi Chen** join us as a phd.
- *2025.10* 获得 2025全球“AI + 无线电”挑战赛 `三等奖`  
- *2025.09* 中海油服-南开大学-东南大学 `基于地震勘探数据的高效压缩解压` 项目顺利结题
 

<span class='anchor' id='-publications'></span>

# 📝 Publications

---
<div class='paper-box'><div class='paper-box-image'><div><div class="badge">TCCN 2026</div><img src='images/TCCN-szy.png' alt="sym" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

- `Z. Song` et al., "Diffusion-Based Spatio-Temporal Channel Prediction via Non-Stationarity Decoupling," in IEEE Transactions on Cognitive Communications and Networking, vol. 12, pp. 7647-7661, 2026, doi: 10.1109/TCCN.2026.3685404.
<!-- (JCR:Q2; IF:3.847)   -->
[[IEEE Xplore]](https://ieeexplore.ieee.org/abstract/document/11488306)
</div>
</div>
---
<div class='paper-box'><div class='paper-box-image'><div><div class="badge">TCOM 2026</div><img src='images/TCOM-26-ljq.png' alt="sym" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

- `J. Li`, H. Xu, Y. Xu, X. Zhao, T. Guo and X. Ling, "The Age of Incorrect Information for Multi-User Link Scheduling Over Fading Channels," in *IEEE Transactions on Communications*, vol. 74, pp. 7182-7198, 2026, doi: 10.1109/TCOMM.2026.3681557.
<!-- (JCR:Q2; IF:3.847)   -->
[[IEEE Xplore]](https://ieeexplore.ieee.org/document/11475769)
</div>
</div>

---
<div class='paper-box'><div class='paper-box-image'><div><div class="badge">ICCASP 2026</div><img src='images/ICASSP-2026-szy.png' alt="sym" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

- `Z. Song`, N. Jiang, M. He, X. Zhao and T. Guo, "Channel, Trend And Periodic-Wise Representation Learning for Multivariate Long-Term Time Series Forecasting," ICASSP 2026 - 2026 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP), Barcelona, Spain, 2026, pp. 4821-4825, doi: 10.1109/ICASSP55912.2026.11464481. 
[[IEEE Xplore]](https://ieeexplore.ieee.org/abstract/document/11464481)
</div>
</div>
<div class='paper-box'><div class='paper-box-image'><div><div class="badge">TSIPN 2025</div><img src='images/TSIPN-czh.png' alt="sym" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

- `Z. Chen`, X. Zhang, M. Zhou, Y. Xu, C. Li and B. Wang, "Online Smooth Graph Learning From Incomplete Data," in *IEEE Transactions on Signal and Information Processing over Networks*, vol. 11, pp. 872-887, 2025, doi: 10.1109/TSIPN.2025.3589719.
<!-- (JCR:Q2; IF:3.847)   -->
[[IEEE Xplore]](https://ieeexplore.ieee.org/document/11106815)
</div>
</div>

---

-	`J. Li`, H. Xu, Y. Xu, X. Zhao, T. Guo and X. Ling, "The Age of Incorrect Information for Multi-User Link Scheduling Over Fading Channels," ICC 2025 - *IEEE International Conference on Communications*, Montreal, QC, Canada, 2025, pp. 2144-2149, doi: 10.1109/ICC52391.2025.11161794. 
[[网页]](https://dx.doi.org/10.3390/s22093587) [[预览]](https://github.com/tangjyan/tangjyan.github.io/blob/main/pdf/TangJ-2022-Mechanism%20of%20Magnetic%20Flux%20Leakage%20Detection%20Method%20Based%20on%20the%20Slotted.pdf) [[下载]](/pdf/TangJ-2022-Mechanism%20of%20Magnetic%20Flux%20Leakage%20Detection%20Method%20Based%20on%20the%20Slotted.pdf)

- ~~Xin Jin, Hongbao Ma, `Jian Tang`, Yihua Kang, A Self-Adaptive Vibration Reduction Method Based on Deep Deterministic Policy Gradient (DDPG) Reinforcement Learning Algorithm. *Applied Sciences*. 2022,12(9): 9703. (JCR:Q2; IF:2.838)  ~~

<!-- ### 专利
---
- 康宜华, `唐健`. 一种基于铁磁性板的缺陷检测方法. [[网页]](https://cprs.patentstar.com.cn/Search/Detail?ANE=9IBC8DFA9GCB8DEA6FAA9HHFCICA1BAA9ACB9EGC9GAA6AAA)
- 康宜华, `唐健`. 基于电磁原理的倒角测量装置.[[网页]](http://www.wanfangdata.com.cn/details/detail.do?_type=patent&id=CN201921137073.X)
- 伍剑波, 王杰, 方辉, `唐健`. 一种钢管脉冲磁化漏磁探伤装置. [[网页]](https://kns.cnki.net/KCMS/detail/detail.aspx?dbcode=SCPD&dbname=SCPD2016&filename=CN105334260A&v=)   -->


<!-- <span class='anchor' id='-rewards'></span>

# 🏅 Achievements -->




<span class='anchor' id='-resources'></span>

# 💻 Resources
---
<div class='paper-box'><div class='paper-box-image'><div><div class="badge"></div><img src='images/AIGC_Learning.png' alt="sym" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

- Artificial Intelligence Generated Content (AIGC) Learning Repository @zysongg.
<!-- (JCR:Q2; IF:3.847)   -->
[[Github]](https://github.com/zysongg/AIGC-Learning)
</div>
</div>


<span class='anchor' id='-members'></span>
# 🧑🏻‍💻 Members

- Yinfei Xu (Professor)
- Zhangyao Song (Phd.)
- Jiaqi Li (Phd.)
<a href="https://orcid.org/0009-0002-5554-2504" aria-label="View ORCID record - 0009-0002-5554-2504"><img src="images/ORCID-iD_icon_vector.svg" width="18pt" alt="ORCID iD"/> 0000-0002-2771-9344</a>  |  <a href="https://github.com/Li-Q-keep"><img class="svg" src="images/github-logo.svg" width="18pt"> Li-Q-keep</a>

- Zehao Chen
- Xiaoyi Chen
- Yunfei Gu
- Shile Ni
- Ruodi Zhang
- Chaofeng Qu
- Muyun Zhou

<span class='anchor' id='-join-us'></span>
# 💬 Join Us

**我们的风格**:
重理论，也关心落地；写公式，也跑代码。

如果你也对信息科学和智能系统感兴趣，欢迎报考东南大学徐寅飞老师课题组！！一起探索"信息"的边界。（📩yinfeixu@seu.edu.cn）